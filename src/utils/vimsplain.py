...
mode_change['ex'] = ['Q']
mode_mapping = dict([(command, mode) for mode in mode_change for command in mode_change[mo[ ]{2,}','\t', line)
	...
	for section in section_lines:
		for parts in section:
			...
			if m:                                                                      = 1 # Assume 1 as a default "default"
			expl = expl.replace('N-1', str(default - 1))                             mode, they do nothing anyway
...
d                           ef parse_commands(fixed_lines):
	commands = {}
	...
	for nsection, lines in enumerate(fixed_lines):
		for i, parts in enumerate(lines):
			...
			for j, regexp in enumerate(regexp_texts): # Fix python <-> vim compatibility issues
				regexp_te                           xts[j] = fix_regexp(regexp)
			...
			if nsection > 0 and nsection != 7 and nsection != 8:
				...
				if nsection == 9: # Ex mode c                           ommands
...
parser.add_option('--convert_special', dest='convert', action='store_true', help='Interpret sequences in ang                           le brackets as special characters (e.g. <CR>).')
...
while instr != '':
	if state.mode == 'insert':
		...
		matched, explanation, instr, state = parse(instr, commands, state)
		print('%s\t%s'%(matched, expl                           anation))
